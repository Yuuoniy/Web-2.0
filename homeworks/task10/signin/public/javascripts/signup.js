 ...
 * @Last Modified time: 2017-12-23 09:29:51
 */
...
function checkVaild() {
  ...
  checkPwd(inputs[4].value, inputs[5].value)
  if (!checkUsername(inputs[0].value) || !checkStudentid(inputs[1].value) ||
    !checkPhone(inputs[2].value) || !checkEmail(inputs[3].value) || !checkPwd(inputs[4].value, inputs[5].value)) {
    // res = false
    return false
  }
  return true
...
function checkPwd(pwd, cpwd) {
  if (pwd == "" || pwd == undefined) {
    ...
    return false
  } else if (!pwd.match(/^[a-zA-Z0-9_-]{6,12}$/)) {
    $(".error")[4].innerText = '6-12 characters (only allow letters, digits ,dash and underscore)'
    return false
  }
  if (pwd != cpwd) {
    $('.error')[5].innerText = 'Inconsistent password.'
  }
  return pwd == cpwd;
}
